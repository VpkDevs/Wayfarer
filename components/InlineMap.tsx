...
const InlineMap: React.FC<Props> = ({ route, currentSegmentIndex, totalSegments }) => {
  ...
  useEffect(() => {
    if (route && directionsRendererRef.current && googleMapRef.current && window.google?.maps) {
  ...
  useEffect(() => {
      if (!progressMarkerRef.current || routePathRef.current.length === 0 || !window.google?.maps) return;
